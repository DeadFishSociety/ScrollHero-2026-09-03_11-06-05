...
2. **Window → Package Manager** → search **"Input System"** (by Unity) → **Install**. Unity will prompt to restart and enable it — accept.
3. Go to **Edit → Project Settings → Player → Other Settings → Active Input Handling** and set it to **"Input System Package (New)"** (or "Both" if you still want legacy `Input.*` calls to work elsewhere). The `SwipeInput.cs` script below uses the new Input System, so this needs to be set for it to compile/run correctly.
4. **Window → Package Manager** → make sure **TextMeshPro** is installed (Unity usually prompts you to "Import TMP Essentials" the first time you drag a TMP object into a scene — click yes).
...
### SwipeInput.cs
Attach to `GameManager`. Detects a mouse/touch drag and fires an event with the direction. Uses the **Input System package** (`UnityEngine.InputSystem`) — checks for an active touchscreen first (so it works correctly on a phone build), and falls back to the mouse (so it works in the Editor).
...
Note for Unreal comparison: this polling pattern (checking device state every `Update()`) is the rough equivalent of polling an Enhanced Input action value each Tick, rather than binding a callback — it's simpler to read for a first pass. If you want to convert this to proper Input Actions (the Unity equivalent of Unreal's Input Action assets/Enhanced Input) later, that's a reasonable next step once the base loop works, but isn't necessary to get this running.
